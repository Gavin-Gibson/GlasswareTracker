...
export const normalizePlayerName = (name?: string): string => {
  ...
  if (lower === 'preston beis' || lower === 'preston bies') return 'Preston Bies';
  if (lower === 'ashley gingerich-showalter' || lower === 'ashley gingerich showalter' || lower === 'ashley gingerich' || lower === 'ashley showalter') return 'Ashley Showalter';
  if (lower === 'kayla wu fleming' || lower === 'kayla wu-fleming' || lower === 'kayla wu') return 'Kayla Wu';
  if (lower === 'jordann vigna' || lower === 'jordan vigna' || lower === 'jordi vigna') return 'Jordi Vigna';
  if (lower === 'matthew cole' || lower === 'matt cole') return 'Matt Cole';
  if (lower === 'daniel mcpartland' || lower === 'dan mcpartland') return 'Dan McPartland';
  if (lower === 'ian  golembeski' || lower === 'ian golembeski') return 'Ian Golembeski';
  if (lower === 'alli kauffman' || lower === 'alli kauffman rogers' || lower === 'alli rogers' || lower === 'allie rogers' || lower === 'allie kauffman') return 'Alli Rogers';
  if (lower === 'kalin miramontes' || lower === 'kalin morgan') return 'Kalin Morgan';
  if (lower === 'lukas eisentraeger' || lower === 'lukas eisenträger' || lower === 'lucas eisentraeger' || lower === 'lucas eisenträger') return 'Lukas Eisenträger';
  ...
  if (lower === 'gabe finocchi' || lower === 'gabe finnochi' || lower === 'gabriel finnochi' || lower === 'gabe finnocchi' || lower === 'gabriel finnocchi' || lower === 'gabriel finocchi') return 'Gabriel Finocchi';
  if (lower === 'fred hinkle' || lower === 'fredric hinkle' || lower === 'frederic hinkle') return 'Fredric Hinkle';
  if (lower === 'joshua fragiacomo' || lower === 'josh fragiacomo') return 'Josh Fragiacomo';
  if (lower === 'coleman model' || lower === 'cole model') return 'Cole Model';
  if (lower === 'maxwell model' || lower === 'max model') return 'Max Model';
  if (lower === 'jake martinez' || lower === 'jacob martinez') return 'Jacob Martinez';
  if (lower === 'christopher hornacek' || lower === 'chris hornacek') return 'Chris Hornacek';
  if (lower === 'drew card' || lower === 'andrew card') return 'Andrew Card';
  if (lower === 'pat drucker' || lower === 'patrick drucker') return 'Patrick Drucker';
  if (lower === 'tom hamilton' || lower === 'tommy hamilton' || lower === 'thomas hamilton') return 'Thomas Hamilton';
  if (lower === 'tony rentsch' || lower === 'anthony rentsch') return 'Anthony Rentsch';
  if (lower === 'tony alvino' || lower === 'anthony alvino') return 'Anthony Alvino';
