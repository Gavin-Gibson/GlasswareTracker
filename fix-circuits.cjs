...
async function run() {
  ...
  for (const t of tournaments) {
    ...
    if (detected && detected !== current) {
      console.log(`Updating "${t.name}" -> ${current} to ${detected}`);
      await supabaseRequest(`tournaments?id=eq.${t.id}`, 'PATCH', { Circuit: detected });
      updatedCount++;
    }
  }
  console.log(`Successfully updated ${updatedCount} tournaments.`);
}
run();
